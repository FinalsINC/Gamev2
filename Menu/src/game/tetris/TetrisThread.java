...
public class TetrisThread extends Thread {
	...
	public TetrisThread(SurfaceHolder holder, Context context) {
		...
		mBackground = mResources.getDrawable(R.drawable.solo);

		Kooky = Typeface.createFromAsset(mContext.getAssets(),
				"Fonts/kooky.otf");
	...
	public TetrisThread(SurfaceHolder holder, Context context, String gameString) {
		...
		mNextPiece.set(mGameString[NEXTPIECESHAPE],
				mGameString[NEXTPIECEROTATION]);
	...
	private Bitmap[] generateBlockBitmaps(int blockSize) {
		...
		for (int i = 0; i < 7; i++) {
			...
			blockBitmaps[i] = Bitmap.createBitmap(colors, blockSize, blockSize,
	...
	private void generateFadeBitmaps() {
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 7; j++) {
				...
				// create the bitmap from the color array
				mFadeBitmaps[i][j] = Bitmap.createBitmap(colors, mBlockSize,
						mBlockSize, Bitmap.Config.ARGB_8888);
			}
		}
	}
		...
		if (mFadingLines) {
			...
			if (mBoard.checkCollision(mPiece.getCoords(mPiecePos))) {
				...
				if (currentTime >= (mLastMoveTime + 500)) {
					mBoard.mergePiece(mPiece.getShapeId(),
							mPiece.getCoords(mPiecePos));
	...
	private void reStorePiece() {
		mPiecePos = new Coord(Integer.parseInt(mGameString[PIECE_X].trim()),
				Integer.parseInt(mGameString[PIECE_Y].trim()));
		if (mBoard.checkCollision(mPiece.getCoords(mPiecePos))) {
			endOfGameByLoss();
		} else
			calcDropPos();
	...
	private void endOfGameByLoss() {
		// good place to call methods that need to be done when the game is
		// over. Ie: hs
		...
		mRunning = false;
		mDisplayPiece = false;
		((Activity) mContext).finish(); // finishes the tetrisview which houses
										// this thread - goes back to mm
	...
	private void generateBackgroundBitmap() {
		...
		bitmapBackground = Bitmap.createBitmap(mCanvasWidth, mCanvasHeight,
				Bitmap.Config.ARGB_8888);
		canvasBackground = new Canvas(bitmapBackground); // draw into the bitmap
		mBackground.draw(canvasBackground);
		// clear screen
		/*
		 * paint.setColor(Color.BLACK); paint.setStyle(Style.FILL); r.bottom =
		 * mCanvasHeight; r.right = mCanvasWidth; r.left = 0; r.top = 0;
		 * canvasBackground.drawRect(r, paint);
		 * 
		 * // draw background int gr=(mCanvasHeight/256)+1; for (int gradient =
		 * 0; gradient < 256; ++gradient) { paint.setColor(Color.argb(255,
		 * gradient,0,0)); paint.setStyle(Style.FILL);
		 * canvasBackground.drawLine(0, gradient*gr , mCanvasWidth, gradient*gr,
		 * paint); }
		 * 
		 * // might want to draw a black board behind the opacic board so the
		 * background is not seen. // draw background for board
		 * paint.setColor(Color.BLACK); //paint.setAlpha(50);
		 * paint.setStyle(Style.FILL);
		 * 
		 * canvasBackground.drawRect((mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/
		 * 2, (mCanvasHeight - mBlockSize * TetrisBoard.DISPLAYED_HEIGHT)/2 ,
		 * mBlockSize * 10 + (mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/2,
		 * mCanvasHeight - (mCanvasHeight-mBlockSize *
		 * TetrisBoard.DISPLAYED_HEIGHT)/2 + 1, paint);
		 */
		// draw OPAIC board
		...
		for (int gridLine = 1; gridLine < 12; gridLine++) {
			mStartX = (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH) / 2
					+ mBlockSize * gridLine;
			mStartY = mCanvasHeight
					- mBlockSize
					* TetrisBoard.DISPLAYED_HEIGHT
					- (mCanvasHeight - mBlockSize
							* TetrisBoard.DISPLAYED_HEIGHT) / 2;
			mStopX = (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH) / 2
					+ mBlockSize * gridLine;
			mStopY = mCanvasHeight
					- (mCanvasHeight - mBlockSize
		...
		for (int gridLine = 1; gridLine < 17; gridLine++) {
			mStartX = (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH) / 2;
			mStartY = mCanvasHeight
					- mBlockSize
					* gridLine
					- (mCanvasHeight - mBlockSize
							* TetrisBoard.DISPLAYED_HEIGHT) / 2;
			mStopX = mBlockSize * 12
					+ (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH) / 2;
			mStopY = mCanvasHeight
					- mBlockSize
					* gridLine
					- (mCanvasHeight - mBlockSize
		...
		canvasBackground.drawRect((mCanvasWidth - mBlockSize
				* TetrisBoard.WIDTH) / 2, (mCanvasHeight - mBlockSize
				* TetrisBoard.DISPLAYED_HEIGHT) / 2, mBlockSize * 12
				+ (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH) / 2 + 1,
				mCanvasHeight
						- (mCanvasHeight - mBlockSize
								* TetrisBoard.DISPLAYED_HEIGHT) / 2 + 1, paint);
		/*
		 * // draw progress bar boarder Display display = ((WindowManager)
		 * mContext
		 * .getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay(); int
		 * mOrientation = display.getOrientation(); paint.setColor(Color.WHITE);
		 * paint.setStyle(Style.STROKE); RectF p1 = new RectF();
		 * if(mOrientation==0||mOrientation==2){ p1.left = mCanvasWidth/7;
		 * p1.right = mCanvasWidth- mCanvasWidth/7; p1.bottom =mCanvasHeight*3/4
		 * + mBlockSize * TetrisBoard.DISPLAYED_HEIGHT/4 + 8 ; p1.top =
		 * p1.bottom - 8; mUnitHorizontal=(p1.right-p1.left)/10; } else{ p1.left
		 * = 3*(mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/8; p1.right =
		 * p1.left+8; p1.bottom =mCanvasHeight - (mCanvasHeight-mBlockSize *
		 * TetrisBoard.DISPLAYED_HEIGHT)/2 +1; p1.top=(mCanvasHeight -
		 * mBlockSize * TetrisBoard.DISPLAYED_HEIGHT)/2;
		 ...
		 * //draw play text
		 * 
		 * Paint paintText2 = new Paint(); paintText2.setColor(Color.WHITE);
		 * paintText2.setTextAlign(Paint.Align.CENTER);
		 * paintText2.setTypeface(Kooky); paintText2.setTextSize(13); float []
		 * pos = new float[3]; pos[0]=
		 * (mCanvasWidth-(mBlockSize*10))/2+mBlockSize*11; pos[1]=
		 * mBlockSize*12; pos[2] = mBlockSize*11; paintText2.setAntiAlias(true);
		 * 
		 * //draw Level canvasBackground.drawText("Level",
		 * mCanvasWidth-(mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/4,
		 * pos[1]-6*mBlockSize, paintText2);
		 * 
		 * //draw Lines canvasBackground.drawText("Lines",
		 * mCanvasWidth-(mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/4,
		 * pos[1]-9*mBlockSize, paintText2);
		 * //canvasBackground.drawText("Lines", (mCanvasWidth/2)+5*mBlockSize,
		 * mBlockSize, paintText2);
		 * 
		 * //draw Score canvasBackground.drawText("Score",
		 * (mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/4, pos[1]-9*mBlockSize,
		 * paintText2); //canvasBackground.drawText("SCORE",
		 * (mCanvasWidth/2)+5*mBlockSize, mBlockSize, paintText2);
		 * 
		 * //draw High Scores //canvasBackground.drawText("High Score",
		 * (mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/4, pos[1]-6*mBlockSize,
		 * paintText2); canvasBackground.drawText("High",
		 * (mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/4, pos[1]-6*mBlockSize,
		 * paintText2); canvasBackground.drawText("Score",
		 * (mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/4, pos[1]-5*mBlockSize,
		 * paintText2); //canvasBackground.drawText("High Score",
		 * (mCanvasWidth/2)-5*mBlockSize, mBlockSize, paintText2);
		 * 
		 * //draw Next canvasBackground.drawText("Next",
		 * mCanvasWidth-(mCanvasWidth-mBlockSize*TetrisBoard.WIDTH)/4,
		 * pos[1]-3*mBlockSize, paintText2);
		 */
	}
	...
	private void doDraw(Canvas canvas) {
		...
		if (mDisplayPiece && !mFadingLines) {
			...
			for (int i = 0; i < 4; i++) {
				if (dropCoords[i].y < TetrisBoard.DISPLAYED_HEIGHT) {
					...
					int screenY = mCanvasHeight
							- (dropCoords[i].y * mBlockSize);
					r.left = screenX
							+ (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH)
							/ 2 + 1;
					r.right = screenX + mBlockSize - 2
							+ (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH)
							/ 2 + 1;
					r.bottom = screenY
							- 2
			...
			for (int i = 0; i < 4; i++) {
				if (pieceCoords[i].y < TetrisBoard.DISPLAYED_HEIGHT) {
					...
					int screenY = mCanvasHeight
							- (pieceCoords[i].y * mBlockSize);
					r.left = screenX
							+ (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH)
							/ 2 + 1;
					r.right = screenX + mBlockSize
							+ (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH)
							/ 2 + 1;
					r.bottom = screenY
							- (mCanvasHeight - mBlockSize
		...
		if (mOrientation == 0 || mOrientation == 2) {
			...
			r1.right = mCanvasWidth / 7 - 1 + mUnitHorizontal
					* (mTotalLines % 10);
			r1.bottom = mCanvasHeight * 3 / 4 + mBlockSize
					* TetrisBoard.DISPLAYED_HEIGHT / 4 + 8;
			r1.top = r1.bottom - 8;
		} else {
			r1.left = 3 * (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH) / 8;
			r1.right = r1.left + 8;
			r1.bottom = mCanvasHeight
					- (mCanvasHeight - mBlockSize
							* TetrisBoard.DISPLAYED_HEIGHT) / 2 + 1 - 1;
		...
		// draw next piece
		/*
		 * int nextPieceX = (int) pos[0]; int nextPieceY = (int) (pos[1] +
		 * 7*mBlockSize);
		 */
		int nextPieceX = (int) (mCanvasWidth
				- (mCanvasWidth - mBlockSize * TetrisBoard.WIDTH) / 4 - 5);
	...
	//		canvas.drawBitmap(mSmallBlockBitmaps[nextShapeId - 1], null, r,
		//			null);
		}

		// draw arrow image
		if (mTouchType == 2)
			mArrow.draw(canvas); // TODO: has the coordinates for the
									// MotionEvent been removed?
	}
		...
		synchronized (mSurfaceHolder) {
			...
			if (mTouchType == 1) {
				...
				case MotionEvent.ACTION_MOVE:
					if (X < mXDown - 5 || X > mXDown + 5 || Y > mYDown + 5
							|| Y < mYDown - 5) {
